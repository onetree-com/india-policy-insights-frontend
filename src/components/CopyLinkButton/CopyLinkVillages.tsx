...
interface Props {
  ...
  divisions?: any[];
  lang: string;
  atlasDivisions?: { stateId: number; districtId: number; villages: any[] };
  dataView?: DataViews;
  deciles?: Array<number>;
}

const CopyLinkVillagesButton: FC<Props> = ({
  buttonStyles,
  indicatorsCategory,
  indicators,
  subregionId,
  ...
  const handleCopyLink = async () => {
    ...
    try {
      ...
      if (divisions && divisions.length > 0) {
        baseUrl += `&subregionIds=${encodeVillages(divisions)}`;
      }
      if (atlasDivisions && atlasDivisions.villages.length > 0) {
        baseUrl += `&subregionIds=${atlasDivisions.stateId}-${
          atlasDivisions.districtId
        },${encodeDivisions(atlasDivisions.villages)}`;
      }
      if (deciles && deciles.length > 0 && deciles.length < 10) {
...
export default CopyLinkVillagesButton;
